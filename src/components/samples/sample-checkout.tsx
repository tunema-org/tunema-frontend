...
import License from '../popout/license'
...
const SampleCheckout = (props: SampleProps) => {
    ...
    <>
      ...
      <section className="w-auto max-w-[736px]">
        <div className="group grid md:grid-cols-6 grid-cols-5 items-center gap-10 hover:bg-primary-green/10 pr-2 font-body p-4">
          ...
          <p className="text-left">{props.time}</p>
          <p className="text-left md:block hidden">{props.bpm}</p>
          <button
            onClick={(e) => {
              setIsLicense((prev) => !prev)
              e.stopPropagation()
            }}
            className="flex font-medium text-dark-green hover:underline justify-start items-start"
          >
            <svg
              width="24"
              height="24"
